...
from pya.domain.errors import InputValidationError, SourceAccessError
from pya.domain.model import SourceUnit, SourceUnitId
from pya.domain.ports import SourceRepository
...
class FileSystemSourceRepository(SourceRepository):
    def load_file(self, path: str) -> SourceUnit:
        ...
        if source_path.suffix != ".py":
            raise InputValidationError(f"expected a .py file, got: {source_path}")

        return self._load_source_unit(source_path)

    def list_python_sources(self, root_path: str) -> tuple[SourceUnit, ...]:
        ...
        source_paths = tuple(sorted(path for path in root.rglob("*.py") if path.is_file()))
        if not source_paths:
            raise InputValidationError(f"no .py files found under: {root}")
